...
```
dbx execute --cluster-name=<name of interactive cluster> --job=db4ml-e2e-mlops-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=db4ml-e2e-mlops-sample-integration-test --files-only
dbx launch --job=db4ml-e2e-mlops-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `db4ml_e2e_mlops/__init__.py` file and tag the current code version:
```
